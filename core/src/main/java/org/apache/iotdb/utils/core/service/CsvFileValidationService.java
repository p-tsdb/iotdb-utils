...
public class CsvFileValidationService implements FileValidationService {
    ...
    public void doDataValidation(List<CSVRecord> allList, List<String> timeseries,Map<String,String> headMap, Session session, ValidationType type) throws StatementExecutionException, IoTDBConnectionException {
    ...
    protected void doValidateData(Map<Long, CSVRecord> recordMap, Long begin, Long end, List<String> timeseries,Map<String,String> headMap,Session session, ValidationType validationType) throws StatementExecutionException, IoTDBConnectionException {
        int dataSetCount = 0;
        ...
        for(int i = 0; i < timeseries.size(); i++){
            if(!timeseries.get(i).equals(columnNameList.get(i+1))){
                assert false:"datavalidation failed, because the num of timeseries in the csv is more than the num of the database; the timeseries is :" + timeseries.get(i);
            }
        }
        ...
        while (dataSet.hasNext()){
            ...
            if(recordMap.get(sdataRecord.getTimestamp()) != null){
                ...
                if(validationType.equals(ValidationType.EQUAL)){
                    throw new AssertionError(" data validation failed, the num of the cvs file is not equals the nums in the database;Time:"+sdataRecord.getTimestamp());
                }
                continue;
            }
        }
        if(recordMap.size() != 0){
            throw new AssertionError(" data validation failed, the num of the cvs file more than the nums in the database;");
        }
        if(dataSetCount == 0){
    ...
    protected SessionDataSet getSessionDataSet(List<String> timeSeries, Long begin, Long end, Session session)
            throws StatementExecutionException, IoTDBConnectionException {
        return session.executeRawDataQuery(timeSeries, begin, end + 1);
    }

    private void getTimeseries(CsvFileTransParser parser, CSVRecord record,List<String> timeseries,Map<String,String> headMap) {
        ...
        for (String headerNameFull : headerNames) {
            ...
            if (!headerNameFull.equals("Time")) {
                String buff = timeSerie + headerName.substring(headerName.lastIndexOf(".") == -1 ? 0 : headerName.lastIndexOf("."), headerName.length());
                timeseries.add(buff);
    ...
    private static boolean compare(String recordValue, Field field, String type) throws StatementExecutionException {
        // csv导入的规则
        if ((recordValue == null || "".equals(recordValue)) && field.getObjectValue(TSDataType.valueOf(type)) == null) {
            return true;
        }
        switch (type) {
            case "TEXT":
